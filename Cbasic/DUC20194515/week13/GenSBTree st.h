...
treetype search(keytype x, treetype root){
  ...
  else if(strcmp(root->key.model, x.model)==0) // found
    return root;
  else if(strcmp(root->key.model, x.model)>0)
...
void insertnode(keytype x, treetype *root){
  ...
  else if(strcmp(root->key.model,x.model)<0) insertnode(x, &(*root)->left);
  else if (strcmp(root->key.model,x.model)>0) insertnode(x, &(*root)->right);
}
